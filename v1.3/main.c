...
int create_socket(char*, BIO*, char*);
void print_ip(struct hostent*);
void get_host_n_path(char*, char*, int, char*, int);
int crawler(SSL*, char*, char*, char*, char*, int, char*, int*);
...
//"https://www.openfind.com.tw/taiwan/", "test_web/", "5"
...
int main(int argc, char* argv[])
{
    ...
    fprintf(fptr, "%s%s\n", padding, argv[1]);              //argv[1] 在 sprintf 這裡不會有 '\0'??
    fflush(fptr);

    //fwrite(padding, PADDING_SIZE, 1, fptr);
    //fwrite(argv[1], strlen(argv[1]) + 1, 1, fptr);        //記得'\0'也要複製
    fclose(fptr);
...
void child_cycle()
{
    ...
    sprintf(record_file, "PID-%d%s", getpid(), ".txt");
    ...
    while(1)
    {
        ...
        if (url_loc)
        {
            ...
            int result = crawler(SI->ssl, host, path, record_file, url_move, SI->server, content, &content_size);
            while (url_move[0])
            {
                ...
                //make_file_name(url_file, host, path);
                result = crawler(SI->ssl, host, path, record_file, url_move, SI->server, content, &content_size);
            }
            ...
            printf("---------------\nlink_buf_size: %d\n---------------\n", link_buf_size);
            link_buf = realloc(link_buf, link_buf_size);
            ...
            if (link_buf[0])     //將網址寫回 contents_file
            {
                ...
                while (curr_ptr < link_buf + link_buf_size)
                {
                    ...
                    if (link_end == NULL)
                        break;
                    char link_size = link_end - curr_ptr;
                    char link[link_size];
                    strncpy(link, curr_ptr, link_size);
                    char* repeat_link = strstr(contents_buf, link);
                    if (repeat_link == NULL)
                    {
...
void ssl_connect(char* url, ssl_info* SI, char* host)
{
   ...
   * ---------------------------------------------------------- */
    ...
    printf("----------\nBUG 111111111111111111\n----------\n");
    SI->outbio = BIO_new_fp(stdout, BIO_NOCLOSE);
    printf("----------\nBUG 222222222222222222\n----------\n");
   ...
   * ---------------------------------------------------------- */
    ...
    SI->server = create_socket(url, SI->outbio, host);
    if(SI->server != 0)
 ...
 * ---------------------------------------------------------- */
int create_socket(char url_str[], BIO* out, char* hostname)
{
    int sockfd;
    //char hostname[256] = "";
    char portnum[6] = "443";
   ...
   * ---------------------------------------------------------- */
    //strncpy(hostname, strstr(url_str, "://") + 3, sizeof(hostname));    //前端 :// 以前去掉，儲存在 hostname
...
int crawler(SSL* ssl, char* host, char* path, char* record_file, char* url_move, int server, char* content, int* content_size)
{
...
void search_link_in_file(char* url_file, char* link_buf)
{
...
void general_case(SSL* ssl, char* response, FILE* fptr, int server, char* content, int* content_size)
{
             ...
             (transfer_mode = strstr(response, "content-length: ")))
    {
        ...
        content_length -= *content_size - (size_loc - response);    //content_length: 前不計
...
void search_link(char* content, int* content_size, char* link_buf, int* link_buf_size)
{
    ...
    char path[MAX_URL_SIZE];
    char link_file[MAX_NAME_SIZE];
    char* curr_ptr = link_buf;                          //用來儲存
    char* link_loc = strstr(content, "href=\"http");    //用來查詢
#ifdef DEBUG
    printf("curr_ptr (start): %p\n", curr_ptr);
#endif
...
#ifdef DEBUG
    *curr_ptr++ = '\0';
    printf("curr_ptr (end): %p\n", curr_ptr);
    printf("link_buf:\n%s\n", link_buf);
#endif
    *link_buf_size = curr_ptr - link_buf;
}
